...
class RemotePhotoData {
  ...
  Future<void> addPhotoForCamera(bool forCamera) async {
    final ImagePicker picker = ImagePicker(); //получение изображения
    ...
    try {
      await supabase.storage
          .from('user_images')
          .upload(fileName, imageFile); // сохраниение изображения в supabase
      ...
      final ImageModel imageModel = ImageModel(
        imageUrl: imageUrl,
      ); // сохранение класса изображения в firestore
  ...
  Future<void> deleteImageById(ImageModel userModel) async {
    try {
      final userId = getUserId();
      final String imagePath = getPathFromUrl(userModel.imageUrl);
      await FirebaseFirestore.instance
          .collection('users')
          .doc(userId)
          .collection('images')
          .doc(userModel.id)
          .delete();
      await supabase.storage.from('user_images').remove([imagePath]);
    } catch (e) {
      throw Exception('Ошибка получения фото: $e');
    }
  }
